...
let push = Stack.push
let pop = Stack.pop
let registers = Hashtbl.create 123456;;


let ord thechar = int_of_char thechar
type binop_t = bigint -> bigint -> bigint

let print_number number = print_bigint_69 number


let print_stackempty () = fprintf stderr "dc: stack empty\n"

let executereg (thestack: stack_t) (oper: char) (reg: int) =
    try match oper with
        | 'l' -> push (Hashtbl.find registers reg) thestack
        | 's' -> Hashtbl.add registers reg  (pop thestack) 
        | _   -> printf "0%o 0%c is --- unimplemented\n%!" 
            (ord oper) (chr reg)
